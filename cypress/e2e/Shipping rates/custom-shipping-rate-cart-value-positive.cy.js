...
describe('Checky Pro - Shipping Rate Cart Value Positive Validation (Single Item, Qty 1)', () => {
    ...
    it('Should create a unique €100-€200 rule, add 1 Laptop (Qty 1), and pass if the shipping rate displays', () => {
        ...
        const uniqueName = `pos-rate-${timestamp}`;

        // Exactly ONE product tracking 'Laptops' at quantity 1
        ...
        cy.log('Step 4: Opening Shopify storefront origin to add 1 Laptop...');
        ...
        // Verify checkout total amount falls between 100 and 200
        ...
        // --- 6. CONDITIONAL VERIFICATION (PASS IF DISPLAYED, FAIL IF NOT) ---
        cy.log('Step 6: Confirming custom shipping rate visibility metrics...');
        ...
        cy.get('body').then(($body) => {
            ...
            if (isShippingMethodShowing) {
                // If it displays, the test passes
                cy.log(`✅ TEST PASSED: Total is between 100 and 200, and the shipping rate "${uniqueName}" is displaying correctly.`);
            } else {
                // If it does not display, the test fails
                throw new Error(`TEST FAILED: Total is between 100 and 200, but the unique shipping rate "${uniqueName}" did not display.`);
            }
        });
    });
});