...
describe('Checky Pro - Shipping Rate Creation & Storefront 2-Item Shipping Method Validation', () => {
    ...
    it('Should create a custom shipping rate linked to First Name, add 2 items, and verify at checkout', () => {
        ...
        if (typeof settingsPage.createShippingRate === 'function') {
            ...
            settingsPage.createShippingRate({ name: customShippingName, minQty: '1', maxQty: '2', price: '10' });
        } else {
            ...
            cy.contains('div, label, span', 'Minimum quantity').parent().find('input').first().clear().type('1');
            cy.contains('div, label, span', 'Maximum quantity').parent().find('input').last().clear().type('2');
        ...
        cy.origin(storeUrl, { args: { PRODUCTS_TO_ADD } }, ({ PRODUCTS_TO_ADD }) => {
            ...
            PRODUCTS_TO_ADD.forEach((product, index) => {
                ...
                cy.wait(3000); // Prevents asynchronous Shopify token session drop errors
            ...
            if (typeof sf.goToCheckout === 'function') {
                ...
                cy.wait(4000); // Allows layout rule parsing engine scripts to bind successfully
        ...
        cy.contains('div, h2, h3, span', /Shipping method/i)
            .parent()
            .within(() => {
                cy.contains('div, span, label, p', new RegExp(customShippingName, 'i'), { timeout: 15000 }).should('be.visible');
                cy.contains('div, span, label, p', /10\.00/i, { timeout: 15000 }).should('be.visible');
            });
    });
});