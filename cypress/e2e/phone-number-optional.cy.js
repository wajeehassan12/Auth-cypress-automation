...
describe('Checky Pro - Optional Phone & Payment Load Validation', () => {
    ...
    it('Should enforce Optional Phone setting and verify payment methods load without a phone number', () => {
        ...
        if (typeof settingsPage.configurePhoneSetting === 'function') {
            ...
            cy.contains('button, label, span', 'Optional', { timeout: 15000 })
                .should('be.visible')
                .click();
        ...
        // Explicitly wait for payment SDK and gateways to finish initializing (Part 1, Rule 2 & Part 2, Rule 11)
        cy.wait('@initPayment', { timeout: 30000 })
            .its('response.statusCode')
            .should('eq', 200);

        // Select country and verify the visible text of the selected option
        ...
        // Verify optional phone field layout without { force: true }
        cy.get('body').then(($body) => {
            if ($body.find('input#phone:visible').length > 0) {
                cy.get('input#phone')
                    .should('be.visible')
                    .clear()
                    .should('have.value', '')
